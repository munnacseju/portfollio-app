          ...
          return (
            <div key={cat}>
              <div className="flex items-center gap-6 mb-12">
                <h3 className="text-4xl font-black uppercase tracking-tighter whitespace-nowrap">{cat}</h3>
                <div className="h-[2px] bg-zinc-900 flex-grow"></div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                {catProducts.map((product) => (
                  <div key={product.id} className="group cursor-pointer">
                    <div className="aspect-[4/5] bg-zinc-900 rounded-2xl overflow-hidden mb-6 relative border border-zinc-800 group-hover:border-zinc-500 transition-colors shadow-2xl">
                      {product.images?.[0] ? (
                        <img 
                      ...
                      {product.discount > 0 && (
                        <div className="absolute top-4 left-4 bg-white text-black text-[10px] px-3 py-1 rounded-full font-black uppercase shadow-xl z-10">
